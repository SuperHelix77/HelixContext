...
def main(root):
    ...
    prior = sorted(HERE.with_name('native-output-v1').joinpath('artifacts').glob('*/*/probe-*.py')) + sorted(HERE.with_name('native-output-v2').joinpath('artifacts').glob('*/*/probe-*.py'))
    manifest['prior_probe_files'] = [str(p.relative_to(HERE.parent)) for p in prior]
    for p in prior: manifest['shared_files'][str(p.relative_to(HERE.parent))] = sha(p.read_bytes())
