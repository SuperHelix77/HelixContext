"""Regrade both Sol artifacts and cross-apply the observed semantic probe."""
import hashlib
import json
...
def verify(destination=None):
    ...
    native_probes = sorted(FILES.glob('*/*/probe-*.py'))
    prior_probes = [HERE.parent / name for name in manifest.get('prior_probe_files',[])]
    probes = native_probes + prior_probes
    rows = []; identities = set()
        ...
        for row in report['rows']:
            ...
            with tempfile.TemporaryDirectory(prefix='helix-native-final-check-') as tmp:
                ...
                for probe in probes:
                    ...
                    receipts.append({'probe':str(probe.relative_to(HERE.parent)), 'exit':0,'stdout':p.stdout.decode(),'stderr':p.stderr.decode()})
    ...
    result = {'classification':'Offline cross-replay of finite behavioral checks; no universal capability claim',
              'artifacts':2,'public_tests_each':23,'independent_cases_each':94,'semantic_probe_programs_each':len(probes),
              'current_native_probe_programs':len(native_probes),'prior_observed_probe_programs':len(prior_probes),
              'all_replayed_semantic_probes_pass_on_all_artifacts':True,'native_model_calls':0,'rows':rows}
    if destination:
