...
class Memory:
    ...
    def search(self,project,query,limit=10,match_mode='all'):
        identity(project)
        if (not isinstance(query,str) or type(limit) is not int or not 1<=limit<=100 or
                not isinstance(match_mode,str) or match_mode not in ('all','any')):
        ...
        match=operator.join('"'+word+'"' for word in words)
        with self.db() as db:
            db.execute('BEGIN')
