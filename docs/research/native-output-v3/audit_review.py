...
def audit(root):
    ...
    successful=[];failed=[]
    for call in calls:
        request_ref,phase,prepared_ref,response_ref=call;assert phase=='COMPLETED'
        ...
        if response['success']:successful.append(call)
        else:
            assert prepared_ref is None
            assert json.loads(response['contentItems'][0]['text'])['status']=='CHECKS_FAILED'
            failed.append(request['callId'])
    assert len(successful)==1 and len(failed)==1
    request_ref,phase,prepared_ref,response_ref=successful[0]
    ...
    scope=json.loads(store.get(packet['scope']['evidence_ref']))
    assert scope['files']['workflow_memory.py']==sha(actual) and scope['status_z']==' M workflow_memory.py\0'
    assert scope['diff_check_exit']==0 and all(r['exit_code']==0 for r in scope['commands'])
    for name,ref in scope['files'].items():assert sha((root/'on'/name).read_bytes())==ref
    initial=json.loads((root/'on-initial-scope.json').read_text())
    assert initial['status_z']=='' and initial['head']==scope['head']
    assert packet['scope']['source_bound'] and packet['scope']['caller_declared_protected_scope_rechecked']
    ...
    value={'classification':'Native diff receipt bound and independently applied; capability/economics separate',
           ...
           'failed_calls_preserved':len(failed),'actual_scope_evidence':'PASS','diff_bytes':len(diff),'receipt_bytes':len(response['contentItems'][0]['text'].encode()),
