...
from review_tool import ReviewEditTool as BoundEditTool
from bound_edit_tool import SPEC
...
def prepare(root):
    ...
    m = {'model': MODEL, 'effort': EFFORT, 'order': order, 'max_semantic_turns_per_arm': 2,
         'blind_retries': 0, 'classification': 'V2 fresh N=1 known maintenance task: literal source blocks and bound diff receipt; not cohort qualification', 'source_presentation': 'literal_blocks_v1', 'review_receipt': 'bound_diff_v1',
    ...
    frozen = [Path(__file__), HERE / 'audit_native.py', HERE.with_name('native-output-v1') / 'test_bound_edit_tool.py', HERE.with_name('native-output-v1') / 'bound_edit_tool.py', HERE / 'review_tool.py', HERE / 'test_review_tool.py', REPO / 'engine/prototype/source_packet.py', HERE / 'PREREG.md', INPUT / 'TASK.md', INPUT / 'oracle.py', INPUT / 'OFFLINE_CALIBRATION.json',
