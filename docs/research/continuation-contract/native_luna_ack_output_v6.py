...
def prepare(root,prior):
 ...
 save(root/'manifest.json',{'model':'gpt-5.6-luna','effort':'high','prior':str(prior),'retries':0,'max_native_turns':1,'classification':'Post-result development candidate; reused immediately preceding persistent Luna control. Not fresh paired causal ablation.','changes':['caller prepares source roles and resolved field proposal','model reviews raw evidence and proposal; emits verdict or semantic escalation'],'stop':'One attempt; preserve failed predecessor and all costs. No retry. Semantic grade required.','sha256':{str(p):sha(p) for p in paths}});print(sha(root/'manifest.json'))
...
def run(root):
 ...
 try:
  ...
  records=[json.loads(line)['event'] for line in selected]
  bases=[e for e in records if e.get('data',{}).get('kind')=='authoritative policy'];assert len(bases)==1,'ambiguous base: native review required'
  source_roles={'policy':bases[0]['event_id'],'amendments':[e['event_id'] for e in records if e.get('data',{}).get('kind')=='authoritative amendment'],'rejected':[e['event_id'] for e in records if e.get('data',{}).get('kind')=='untrusted vendor note']}
  proposed_fields={k:bases[0]['data'][k] for k in FIELDS}
  for e in sorted(records,key=lambda e:e['turn']):
   if e['event_id'] in source_roles['amendments']:
    assert e['data'].get('scope')=='Only approver_group changes; all other policy fields remain as originally recorded.','unsupported scope'
    proposed_fields['approver_group']=e['data']['approver_group']
  with Session(m['model'],cwd,cwd/'receipts/run',skill=cwd/'.agents/skills/helixcontext/SKILL.md') as s:
   ...
   prompt='Caller completed memory, skill registration and persistence; caller owns bookkeeping. Full history remains in history.jsonl. High reasoning and ordinary tools remain available.\nReview the proposed source roles and resolved policy against the exact raw records and original request. Caller prepared these mechanically; they may be wrong. You retain semantic authority. If the proposal is adequate, emit only INSUFFICIENT_APPROVERS, WRONG_GROUP or AUTHORIZED as your reason/verdict. If any source, scope, field or obligation is inadequate, emit REVIEW_REQUIRED and explain what needs semantic correction. Caller supplies the complete final JSON from the reviewed proposal and your verdict; no identifiers or JSON need be regenerated.\n'+'\nOriginal request:\n'+json.dumps(events[-1])+'\nProposed source roles and fields:\n'+json.dumps({'roles':source_roles,'fields':proposed_fields})+'\nExact subject-matching records; full archive SHA256='+h+':\n'+packet.decode()
  ...
  verdict=answer.strip();assert verdict in ('INSUFFICIENT_APPROVERS','WRONG_GROUP','AUTHORIZED'),'semantic review required: '+verdict
  decision={**source_roles,'reason':verdict,'authorized':verdict=='AUTHORIZED'}
