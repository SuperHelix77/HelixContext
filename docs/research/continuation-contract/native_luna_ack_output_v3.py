"""Second authorized output follow-up; existing control reused and labelled."""
...
def prepare(root,prior):
 ...
 save(root/'manifest.json',{'model':'gpt-5.6-luna','effort':'high','prior':str(prior),'retries':0,'max_native_turns':1,'classification':'Post-result development candidate; reused immediately preceding persistent Luna control. Not fresh paired causal ablation.','changes':['explicit source roles replace order-dependent list','bounded scope validation','model reason code plus caller explanation'],'stop':'One attempt; preserve failed predecessor and all costs. No retry. Semantic grade required.','sha256':{str(p):sha(p) for p in paths}});print(sha(root/'manifest.json'))
...
{"authorized":boolean,"policy":base_policy_event_number,"amendments":[applicable_amendment_event_numbers],"rejected":[relevant_non_authoritative_event_numbers],"reason":"INSUFFICIENT_APPROVERS|WRONG_GROUP|AUTHORIZED"}.
