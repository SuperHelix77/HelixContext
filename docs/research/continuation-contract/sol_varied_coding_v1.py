"""Sol High transfer of the frozen caller/source-only coding boundary.
...
REPO = HERE.parents[2]
MODEL = 'gpt-5.6-sol'
BASE = REPO / 'engine/profiles/sol-coding-transfer-v1/base.md'


@contextmanager
def configured(root):
    root = Path(root).resolve()
    old = shared.BASE, shared.BASE_HASH, shared.verify, shared.observed_session.Session, shared.save
    def verify(manifest):
        old[2](manifest)
    ...
    try:
        ...
        shared.BASE, shared.BASE_HASH, shared.verify, shared.observed_session.Session, shared.save = old
...
def prepare(root):
    ...
    manifest.update(model=MODEL, base_file=str(BASE), classification=
        'Three fresh Sol High pairs on previously exposed development coding contracts; source-only caller transfer; not a population holdout',
        ...
        original_luna_policy_unchanged=True, git_workspaces={}, registration={})
    ...
    for path in [Path(__file__),HERE/'SOL_VARIED_CODING_V1_PREREG.md',Path(workspace.__file__),
...
def audit(root):
    ...
    with configured(root):
        shared.audit(root)
    report=json.loads((root/'sol-audit-raw.json').read_text())
    m=json.loads((root/'manifest.json').read_text())
    ...
    shared.save(HERE/'SOL_VARIED_CODING_V1_RESULT.json',report)
