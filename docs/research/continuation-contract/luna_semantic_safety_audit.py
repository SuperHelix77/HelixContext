...
def audit(root):
    ...
    for row in result['rows']:
        ...
        rows.append({**row, 'segments':len(updates),'commands':len(commands), 'native_sha256':status['native_events_sha256'],'elapsed_seconds':status['elapsed_seconds']})
    paired=[]
    for case in CASES:
    ...
    output=Path(__file__).with_name('LUNA_SEMANTIC_SAFETY_RESULT.json')
