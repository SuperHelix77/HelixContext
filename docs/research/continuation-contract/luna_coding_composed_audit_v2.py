...
def audit(root):
    ...
    assert base.endswith(b'\n') and meta['base_instructions']['text'].encode()==base[:-1]
    ...
    result={'classification':'One adaptive frozen-component composition; references retained, no fresh pair or general release',
            ...
            'base_sha256':hashlib.sha256(base).hexdigest(),'base_bytes':len(base),'native_base_matches_exact_file_without_terminal_lf':True,'native_base_bytes':len(base)-1,'native_base_sha256':hashlib.sha256(base[:-1]).hexdigest(),'audit_amendment':'V1 required final LF preservation; Codex metadata strips exactly that byte. V2 admits only that exact normalization; task graders and native execution unchanged','auditor_sha256':original.cap.kd.sha(Path(__file__)),
