...
def audit(root):
    ...
    for row in results['rows']:
        ...
        if arm == 'on':
            import sqlite3
            from bound_edit_tool import digest, encoded
            store = r.Store(root / 'on-tool/objects'); before = dict(store.metrics)
            with sqlite3.connect(root / 'on-tool/calls.sqlite3') as db:
                calls = list(db.execute('SELECT id,request,phase,prepared,response FROM calls'))
                state = db.execute('SELECT version,source FROM state WHERE id=0').fetchone()
            native_calls = [e['params'] for e in events if e.get('method') == 'item/tool/call']
            assert len({p['callId'] for p in native_calls}) == len(calls)
            for call, request_ref, phase, prepared_ref, response_ref in calls:
                assert phase == 'COMPLETED'
                request = json.loads(store.get(request_ref)); response = json.loads(store.get(response_ref))
                assert request in native_calls and digest(encoded([request['threadId'],request['callId']])) == call
                if response['success']:
                    prepared = json.loads(store.get(prepared_ref))
                    original = store.get(prepared['prior_source'])
                    plan, _ = r.compile_edits(original,prepared['prior_source'],{'edits':request['arguments']['edits']})
                    exact, _ = r.renderer.assemble(store,plan)
                    assert digest(exact)==prepared['output_source']
                    assert json.loads(store.get(prepared['check_ref']))['checks']=='PASS'
            if calls and row['checks']=='PASS': assert state[1] == r.sha(cwd/r.TARGET)
            reconstruction_io = {k:store.metrics[k]-before[k] for k in before}
        else: calls=[]; reconstruction_io=None
        final = row['turns'][-1]['answer'].strip(); assert final
        try: structured=json.loads(final)
        except ValueError: structured=None
        assert structured is None, 'Operation JSON is not a normal final answer for this coding task'
        finals=[i for i,e in enumerate(events) if e.get('method')=='item/completed'
                and e.get('params',{}).get('item',{}).get('type')=='agentMessage'
                and e['params']['item'].get('phase')=='final_answer']
        assert finals and events[finals[-1]]['params']['item']['text']==row['turns'][-1]['answer']
        tool_ends=[i for i,e in enumerate(events) if e.get('method')=='item/completed'
                   and e.get('params',{}).get('item',{}).get('type')=='dynamicToolCall']
        if tool_ends: assert finals[-1]>max(tool_ends)
        ...
        rows.append({'arm': arm, 'model': m['model'], 'effort': m['effort'], 'thread_id': s['thread_id'],
                     ...
                     'raw_tool_calls': idx['calls'], 'engine_tool_calls':len(calls), 'model_written_final_after_tools':True, 'raw_sha256': idx['sha256'], 'raw_bytes': len(raw),
                     ...
                     'store_io': row['store_io'], 'final_answer_semantic_review':'PENDING explicit answer/evidence review', 'post_audit_reconstruction_io': reconstruction_io,
    ...
    report = {'classification': m['classification'], 'N': 1, 'manifest_sha256': r.sha(root / 'manifest.json'),
              ...
              'normal_codex_app_integration': False, 'cohort_complete':False, 'release_qualification':False, 'model': m['model'], 'effort': m['effort'],
              ...
              'limits': ['Composite kernel/skill/internal-tool test with model-written final answer; no isolated mechanism attribution',
    ...
    r.save(root / 'audit.json', report); r.save(r.HERE / 'LUNA_HIGH_RESULT.json', report)
