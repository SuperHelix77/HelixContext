...
def main(root):
    ...
    anatomy = {}
    for model, report_name in [('astra-xhigh', 'ASTRA_XHIGH_RESULT.json'), ('sol-high', 'SOL_HIGH_RESULT.json')]:
        base = root / model; raw_report = (base / 'audit.json').read_bytes()
            ...
            for i, event in enumerate(events):
                ...
                if method == 'item/tool/call':
                    ...
                    if ('Supplemental checks passed:' in output or '12 additional semantic assertions passed:' in output
                            or 'literal-operator probe passed' in output):
