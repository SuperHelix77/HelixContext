"""Offline recovery audit derived from frozen audit.py; see RECONCILIATION.md.
Original native results/status and frozen auditor remain unchanged.
"""
import hashlib
import json
...
def audit(root):
    root = Path(root).resolve(); m = json.loads((root / 'manifest.json').read_text()); r.verify(m)
    results = json.loads((root / 'results.json').read_text())
    assert results['state'] == 'STOPPED_PENDING_AUDIT'
    assert results['error'] == "RuntimeError('Unexpected task-file modification')"
    assert results['manifest_sha256'] == r.sha(root / 'manifest.json')
    original_results_sha256 = r.sha(root / 'results.json')
    assert [x['arm'] for x in results['rows']] == m['order']
    rows = []; identities = set()
    for row in results['rows']:
        arm = row['arm']; cwd = root / arm; run = root / (arm + '-run')
        if arm == 'off': r.scope(cwd, m['git'][arm])
        else:
            assert r.workspace.git(cwd, 'rev-parse', 'HEAD') == m['git'][arm]['initial_commit']
            changed = r.subprocess.check_output(['git', 'status', '--porcelain=v1', '-z', '--untracked-files=all'], cwd=cwd)
            assert set(changed.split(b'\0')) == {b' M workflow_memory.py', b'?? workflow_memory.py.helix-lock', b''}
            lock = cwd / 'workflow_memory.py.helix-lock'
            assert lock.is_file() and not lock.is_symlink() and lock.read_bytes() == b''
            assert row['checks'] == 'PASS' and len(row['turns']) == 1
            publication = row['turns'][0]['publication']
            assert r.sha(cwd / r.TARGET) == publication['sha256']
            assert publication['source_sha256'] == [m['inputs'][r.TARGET]]
            # Complete only missing observer metadata from retained receipts.
            row['source_sha256'] = publication['sha256']
            row['store_io'] = publication['io']
            row['elapsed_seconds'] = json.loads((run / 'status.json').read_text())['elapsed_seconds']
        ...
        assert s['model'] == m['model'] and s['effort'] == 'high' and s['thread_id'] not in identities
        ...
        if arm == 'on': assert idx['calls'] == idx['outputs'] == 0
        ...
        assert starts[0]['model'] == m['model'] and starts[0]['config']['model_reasoning_effort'] == 'high'
        ...
        assert all(x['model'] == m['model'] and x['effort'] == 'high' for x in turns)
        ...
        assert binding['model'] == m['model'] and binding['effort'] == 'high'
        ...
        if arm == 'on' and row['checks'] == 'PASS':
            ...
            text, plan, error = r.proposal(row['turns'][-1]['answer'],
                                         (r.HERE / 'baseline' / r.TARGET).read_bytes(), m['inputs'][r.TARGET])
        ...
        recheck = r.grade(cwd, root / (arm + '-reconciled-audit-check'))
        assert recheck['checks'] == row['checks']
        mem = json.loads((root / (arm + '-memory') / 'receipt.json').read_text())
        rows.append({'arm': arm, 'model': m['model'], 'effort': 'high', 'thread_id': s['thread_id'],
    ...
    report = {'classification': m['classification'], 'N': 1, 'manifest_sha256': r.sha(root / 'manifest.json'),
              ...
              'normal_codex_app_integration': False,
    ...
    report.update(original_runner_state='STOPPED_PENDING_AUDIT',
                  original_results_sha256=original_results_sha256,
                  reconciliation='Narrow offline amendment: exact zero-byte caller-owned publication lock admitted; no model call or task change',
                  clean_frozen_protocol_pass=False,
                  candidate_elapsed_source='Native closed-session receipt; includes in-session caller checks/publication, excludes pre-construction setup',
                  caller_source_inspection_required=True)
    assert r.sha(root / 'results.json') == original_results_sha256
    r.save(root / 'reconciled-audit.json', report); r.save(r.HERE / 'RECONCILED_RESULT.json', report)
