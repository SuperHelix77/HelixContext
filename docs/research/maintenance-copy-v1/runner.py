"""Frozen single Astra maintenance pair; exact edits, no solved caller template."""
import hashlib
import json
import os
...
import config_bound_session as native
...
def prepare(root):
    ...
    inputs = {n: (HERE / 'baseline' / n).read_bytes() for n in FILES[:3]}
    inputs[FILES[-1]] = (HERE / FILES[-1]).read_bytes()
    ...
    m = {'model': MODEL, 'effort': 'high', 'order': order, 'max_semantic_turns_per_arm': 2,
         'blind_retries': 0, 'classification': 'Fresh N=1 existing-code maintenance pair; not earlier coding qualification',
    ...
    frozen = [Path(__file__), HERE / 'audit.py', HERE / 'test_runner.py', HERE / 'PREREG.md', HERE / 'TASK.md', HERE / 'oracle.py', HERE / 'OFFLINE_CALIBRATION.json',
              HERE / 'BASELINE_MANIFEST.json', BASE, CLI, Path('/Users/mert/.codex/AGENTS.md'), memory.CLIENT]
    ...
    frozen += [HERE / 'baseline' / n for n in FILES[:3]] + [HERE / FILES[-1]]
    for arm in ('off', 'on'):
        ...
        frozen += [cwd / n for n in FILES if n != TARGET] + [cwd / 'AGENTS.md', skill, cwd / '.gitignore', root / (arm + '-prompt.txt')]
...
def grade(cwd, destination):
    ...
    for label, args in [('public', [sys.executable, '-B', '-m', 'pytest', '-q', FILES[2], FILES[3]]),
                        ('oracle', [sys.executable, '-B', str(HERE / 'oracle.py'), str(cwd)])]:
        try:
...
def run(root):
    ...
    try:
        for arm in m['order']:
            ...
            with native.Session(MODEL, cwd, root / (arm + '-run'), skill) as session:
