...
def grade(alias):
 ...
 for task in ['Q','A','W']:
  ...
  for condition in ['off','on']:
   p=(WORKSPACE/'outputs/helix-frontier'/alias/task/condition) if task in ['A','W'] and condition=='off' else base/task/condition
   if not (p/'summary.json').exists():result[task][condition]={'status':'pending'};continue
   summary=load(p/'summary.json');checks={}
   try:
    if task=='Q':
     checks['exact_reasoning_answer']=identical(parse_answer(p/'turn-01/answer.txt'),load(ROOT/'evaluator/reasoning-gold.json'))
     checks['original_source_preserved']=load(p/'integrity.json')['source_unchanged']
    elif task=='A':
...
'''
     ...
     checks['existing_suite_executed']=any(e.get('type')=='item.completed' and e.get('item',{}).get('type')=='command_execution' and 'unittest' in e['item'].get('command','') and 'Ran 4 tests' in e['item'].get('aggregated_output','') and 'OK' in e['item'].get('aggregated_output','') for e in events)
    else:
     ...
     if condition=='on':
      checks['caller_owned_state_preserved']=all(x['caller_files_unchanged'] for x in load(p/'integrity.json'))
 ...
 (base/'validation-grading.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
