...
def prepare(alias,arm):
    ...
    if arm=='on':
        ...
        started=time.perf_counter();view=compile_view(raw,TASK)
        prompt+='Helix supplies exact source lines selected by literal request terms. This is partial evidence, not an answer. Use test.log for any missing evidence.\n'+json.dumps(view,separators=(',',':'))
        overhead={'source_bytes_parsed':len(raw),'source_bytes_hashed':len(raw),'preprocessing_seconds':time.perf_counter()-started,'view_bytes':len(json.dumps(view,separators=(',',':')).encode())}
...
def run(alias):
    ...
    for arm in (['on','off'] if alias=='sol' else ['off','on']):
        ...
        row={'answer':answer,'model':MODELS[alias],'arm':arm,'passed':passed,'usage':status['usage'],'elapsed_seconds':status['elapsed_seconds'],'events_sha256':status['events_sha256'],'source_unchanged':sha((cwd/'test.log').read_bytes())==sha(raw_log()),'middleware_application_bytes':overhead}
