"""Observed research session v2: durable raw tool receipts, no inference retries.

Versioned from research_session.py; frozen earlier protocols remain untouched.
Persistent history adds storage overhead and is private research evidence.
"""
...
class Session:
    def __init__(self,model,cwd,out,skill=None):
        ...
        self.config_hash=sha(config.read_bytes()) if config.exists() else None
        try:
            ...
            r=self.rpc.call('thread/start',{'model':model,'cwd':str(self.cwd),'ephemeral':False,'approvalPolicy':'never','sandbox':'workspace-write','config':{'model_reasoning_effort':'high','skills.max_context_tokens':512}})
            save(self.out/'thread-start.json',r)
            if r['model']!=model or r.get('reasoningEffort')!='high' or r['approvalPolicy']!='never':raise ValueError('Native configuration mismatch')
    ...
    def turn(self,text=None,tool_output=None):
        ...
        params={'threadId':self.thread,'model':self.model,'effort':'high','input':[]}
    ...
    def close(self):
        if not self.closed:
            self.rpc.close();self.closed=True
            try:
                current=sha(self.config_path.read_bytes()) if self.config_path.exists() else None
                if current!=self.config_hash:raise ValueError('Configuration changed during observed session')
                ...
                self.capture_status={'error':repr(exc)}
                self.failed=True
            self.record('failed' if self.failed else 'closed');self.normalized.close()
