"""Research-only live-thread continuation; no auto-retry, resume or closure policy."""
...
class Session:
    def __init__(self,model,cwd,out,skill=None):
        ...
        self.cursor=0;self.registration=None
        try:
            ...
            r=self.rpc.call('thread/start',{'model':model,'cwd':str(self.cwd),'ephemeral':True,'approvalPolicy':'never','sandbox':'workspace-write','config':{'model_reasoning_effort':'high','skills.max_context_tokens':512}})
    ...
    def close(self):
        if not self.closed:
            self.rpc.close();self.closed=True;self.record('failed' if self.failed else 'closed');self.normalized.close()
