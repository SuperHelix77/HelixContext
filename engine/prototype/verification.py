...
def verify(store, candidate):
    ...
    for field in ('exit_code', 'timed_out', 'wall_seconds', 'environment_id'):
        if candidate.get(field) != receipt[field]:
            raise ValueError('Receipt mismatch: ' + field)
    if candidate.get('interrupted') != receipt.get('interrupted', False):
    ...
    for field, source in (('command', 'argv'), ('cwd', 'cwd'), ('changed_watched_files', 'changed_watched_files')):
        if field in candidate:
            if candidate[field] != receipt[source]:
                raise ValueError('Receipt mismatch: ' + field)
    ...
    for stream in ('stdout', 'stderr'):
        if candidate['raw'][stream] != receipt[stream]:
        ...
        if projection.get('summary_candidates') != summaries[-2:]:
            raise ValueError('Summary count evidence mismatch')
        emitted = [entry['line'] for entry in projection.get('diagnostics', [])]
        if emitted != diagnostic_lines[:len(emitted)] or projection.get('diagnostic_lines_omitted') != len(diagnostic_lines)-len(emitted):
            raise ValueError('Diagnostic omission accounting mismatch')
        sections = projection.get('failure_section_index', [])
        if projection.get('failure_sections_omitted') != len(section_lines)-len(sections):
        ...
        for index, section in enumerate(sections):
            ...
            if section != {'start': start, 'end': end, 'header': lines[start-1]}:
