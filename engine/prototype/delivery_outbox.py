...
READBACK_VERSION = 'helix.codex-display-readback.v2'
...
class DisplayOutbox:
    ...
    def reconcile(self, ticket, native_thread, *, native_shell=None):
        ...
        A caller may bind an inspected native shell envelope, e.g. ('/bin/zsh',
        ...
        """
        if native_shell is not None and (not isinstance(native_shell, tuple) or len(native_shell) != 2
                or native_shell[0] not in ('/bin/zsh', '/bin/bash', '/bin/sh') or native_shell[1] not in ('-lc', '-c')):
            raise ValueError('Explicit supported native shell envelope required')
        def command_matches(value, expected):
            if native_shell is None:
                return value == expected
            if not isinstance(value, str):
                return False
            try:
                return shlex.split(value) == [*native_shell, expected]
            except ValueError:
                return False
        with self.memory.db() as db:
            db.execute('BEGIN IMMEDIATE')
            ...
            for turn in native_thread.get('turns', []):
                for item in turn.get('items', []):
                    if item.get('type') == 'commandExecution' and command_matches(item.get('command'), packet['command']):
            ...
            evidence = dict(schema=VERSION, readback_version=READBACK_VERSION, native_shell=native_shell,
                            request_hash=ticket['request_hash'], thread_id=packet['thread_id'],
