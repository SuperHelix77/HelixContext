...
import checked_steps,evidence,plan_dependencies as dep
...
def register(store,plan_id,plan_version,*,cwd,steps,files,executables,env_names=(),environment=None):
    ...
    if len(set(env_names))!=len(env_names) or any(not isinstance(n,str) or not re.fullmatch(r'[A-Za-z_][A-Za-z0-9_]*',n) for n in env_names):raise ValueError('Invalid environment names')
