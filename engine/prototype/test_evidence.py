...
import pytest
from evidence import Store,run,packet,reduce_stream
...
def test_exit_semantics_and_raw_stderr(tmp_path):
 cli=Path(__file__).parent/'evidence.py'
 p=subprocess.run([sys.executable,str(cli),'--store',str(tmp_path/'s'),'run','--cwd',str(tmp_path),'--',sys.executable,'-c','import sys;print("failed",file=sys.stderr);sys.exit(7)'],capture_output=True,text=True)
