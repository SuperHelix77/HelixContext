...
def fetch():
    # System trust store; never disable TLS verification. Bounded download and timeout.
    result = subprocess.run(['curl', '--fail', '--silent', '--show-error', '--proto', '=https',
        '--max-time', '15', '--max-filesize', '1000000', SOURCE], capture_output=True, timeout=20, check=True)
...
class Prices:
    ...
    def state(self):
        with self.lock:
            ...
            return {'source':SOURCE,'checked_at':self.checked,'expires_at':self.checked+TTL if self.checked else None,
